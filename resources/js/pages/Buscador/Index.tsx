...
const Index: React.FC<BuscadorProps> = ({ libros, secciones, filters }) => {
  ...
  useEffect(() => {
    ...
    // Búsqueda optimizada con debouncing inteligente
    if (seccionId) {
      if (search.length === 0) {
        // Búsqueda instantánea si está vacío
        handleSearch(search, seccionId);
      } else if (search.length >= 3) {
        // Debouncing de 400ms para búsquedas válidas (balance UX/performance)
        searchTimeoutRef.current = setTimeout(() => {
          handleSearch(search, seccionId);
        }, 400);
      }
      // Si length es 1-2, no hacer nada (esperar a que llegue a 3)
    }
              ...
              <div className="lg:col-span-2">
                ...
                <div className="relative mb-2">
                  <input
                    ...
                    disabled={!seccionId}
                    className={`w-full pl-8 pr-10 py-2.5 rounded-lg border text-gray-900 dark:text-white
                  ...
                  {loading && search.length >= 3 && seccionId && (
                  ...
                  {/* Indicador de estado cuando no está buscando */}
                  {!loading && search && seccionId && (
                    <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
