...
const Index = ({ 
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      ...
      if (searchTerm.trim() !== search) {
        ...
        if (selectedFilters.ciudad) currentParams.set('ciudad', selectedFilters.ciudad);
        if (selectedFilters.pais) currentParams.set('pais', selectedFilters.pais);
  ...
  }, [searchTerm, search, selectedFilters]);

  // Función para manejar el ordenamiento - SOLO NOMBRE
  const handleSort = () => {
    const currentParams = new URLSearchParams(window.location.search);
  ...
  const handleFilterChange = (filterType: string, value: string): void => {
    ...
    if (searchTerm.trim()) currentParams.set('search', searchTerm.trim());
    if (newFilters.ciudad) {
      currentParams.set('ciudad', newFilters.ciudad);
    } else {
      currentParams.delete('ciudad');
    }
    if (newFilters.pais) {
      currentParams.set('pais', newFilters.pais);
    } else {
      currentParams.delete('pais');
    }
  ...
  const paginationData = pagination || {
    current_page: 'current_page' in editoriales ? editoriales.current_page : 1,
    last_page: 'last_page' in editoriales ? editoriales.last_page : 1,
    per_page: 10,
    total: editoriales.total || 0,
    from: 'from' in editoriales ? editoriales.from : null,
    to: 'to' in editoriales ? editoriales.to : null,
    has_pages: (editoriales.total || 0) > 10
  };

  // Usar las opciones desde el servidor (todas las ciudades y países disponibles)
  const uniqueCiudades = all_ciudades || [];
  const uniquePaises = all_paises || [];
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {paginationData.total > 0 && (
          <div className="mb-4 text-sm text-gray-600 dark:text-gray-400">
            Mostrando {paginationData.from || 1} a {paginationData.to || editoriales.data.length} de {paginationData.total} resultados
          </div>
        )}
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[700px] table-fixed">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {editoriales.data.length > 0 ? (
                  editoriales.data.map((editorial, index) => (
                    <tr key={editorial.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-gray-700 dark:text-gray-300 text-sm max-w-xs">
                        <div className="truncate" title={editorial.pais || '-'}>
                          {editorial.pais || '-'}
                        </div>
                      </td>
                      <td className="px-4 py-3 text-gray-700 dark:text-gray-300 text-sm max-w-xs">
                        <div className="truncate" title={editorial.ciudad || '-'}>
                          {editorial.ciudad || '-'}
                        </div>
                      </td>
