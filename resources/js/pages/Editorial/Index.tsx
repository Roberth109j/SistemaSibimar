...
const Index = ({ auth, editoriales, flash, errors = {} }: IndexProps) => {
  ...
  const [currentEditorial, setCurrentEditorial] = useState<Editorial | null>(null);
  ...
  // Actualizar el formulario cuando se selecciona una editorial para editar
  useEffect(() => {
    if (currentEditorial && isEditModalOpen) {
      setFormNombre(currentEditorial.nombre);
      setFormCiudad(currentEditorial.ciudad || '');
      setFormPais(currentEditorial.pais || '');
    } else if (!isEditModalOpen) {
      setFormNombre('');
      setFormCiudad('');
      setFormPais('');
    }
  }, [currentEditorial, isEditModalOpen]);

  // Filtrar editoriales según el término de búsqueda
  ...
  const totalPages = Math.ceil(filteredEditoriales.length / itemsPerPage);

  // Obtener las editoriales para la página actual
  const paginatedEditoriales = filteredEditoriales.slice(
  ...
  const openEditModal = (editorial: Editorial) => {
    setCurrentEditorial(editorial);
    setFormNombre(editorial.nombre);
    setFormCiudad(editorial.ciudad || '');
    setFormPais(editorial.pais || '');
    setIsEditModalOpen(true);
  };

  const openViewModal = (editorial: Editorial) => {
    setCurrentEditorial(editorial);
  ...
  // Manejar la creación de una nueva editorial
  const handleCreate = (e: React.FormEvent) => {
    e.preventDefault();
    setIsProcessing(true);

    router.post('/editoriales', {
      nombre: formNombre,
      ciudad: formCiudad,
      pais: formPais
    }, {
      onSuccess: () => {
        showAlert('success', 'Editorial creada exitosamente');
        closeAllModals();
        setIsProcessing(false);
      },
      onError: () => {
        showAlert('error', 'Ha ocurrido un error al crear la editorial');
        setIsProcessing(false);
      },
      preserveScroll: true
    });
  ...
  // Manejar la actualización de una editorial existente
  const handleUpdate = (e: React.FormEvent) => {
    ...
    router.put(`/editoriales/${currentEditorial.id}`, {
      nombre: formNombre,
      ciudad: formCiudad,
      pais: formPais
    }, {
      onSuccess: () => {
        showAlert('success', 'Editorial actualizada exitosamente');
        closeAllModals();
        setIsProcessing(false);
      },
      onError: () => {
        showAlert('error', 'Ha ocurrido un error al actualizar la editorial');
        setIsProcessing(false);
      },
      preserveScroll: true
    });
      ...
      <div className="max-w-7xl mx-auto relative z-10">
                    ...
                    <tr key={editorial.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex space-x-4">
                          <button
                            onClick={() => openViewModal(editorial)}
                          ...
                          <button
                            onClick={() => openEditModal(editorial)}
        ...
        >
          <form onSubmit={handleCreate} className="space-y-5">
            <div>
              <label htmlFor="create_nombre" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Nombre
              </label>
              <input
                id="create_nombre"
            ...
            <div>
              <label htmlFor="create_ciudad" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Ciudad
              </label>
              <input
                id="create_ciudad"
            ...
            <div>
              <label htmlFor="create_pais" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                País
              </label>
              <input
                id="create_pais"
        ...
        >
          {currentEditorial && (
            <form onSubmit={handleUpdate} className="space-y-5">
              <div>
                <label htmlFor="edit_nombre" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Nombre
                </label>
                <input
                  id="edit_nombre"
            ...
            placeholder-gray-400 focus:border-amber-500 transition-colors duration-200"
              ...
              <div>
                <label htmlFor="edit_ciudad" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Ciudad
                </label>
                <input
                  id="edit_ciudad"
            ...
            placeholder-gray-400 focus:border-amber-500 transition-colors duration-200"
              ...
              <div>
                <label htmlFor="edit_pais" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  País
                </label>
                <input
                  id="edit_pais"
        ...
        {/* Modal para ver detalles de la editorial */}
        <Modal
          ...
          {currentEditorial && (
            <div className="space-y-6">
              <div className="overflow-hidden bg-white dark:bg-gray-800 shadow-md rounded-lg">
                ...
                <div className="border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-600/10 to-indigo-600/10 dark:from-blue-600/20 dark:to-indigo-600/20 px-6 py-5">
                  <div className="flex items-center">
                    <div className="flex-shrink-0 flex items-center justify-center h-14 w-14 rounded-full bg-gradient-to-r from-blue-500 to-indigo-500 text-white shadow-lg">
                      <span className="text-xl font-bold">{currentEditorial.id}</span>
                    </div>
                    <div className="ml-5">
                      <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                        {currentEditorial.nombre}
                      </h3>
                      <p className="text-sm text-gray-500 dark:text-gray-400">ID: {currentEditorial.id}</p>
                    </div>
                  </div>
                </div>
                ...
                {currentEditorial.libros && currentEditorial.libros.length > 0 && (
                    ...
                    <div className="px-6 py-4 bg-gradient-to-r from-blue-600/10 to-indigo-600/10 dark:from-blue-600/20 dark:to-indigo-600/20">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-base font-medium text-gray-900 dark:text-white">
                          Libros de la Editorial ({currentEditorial.libros.length})
                        </h3>
                      </div>
                    </div>
                    <div className="px-6 py-4">
                      <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                        {currentEditorial.libros.map((libro) => (
